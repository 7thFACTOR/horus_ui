	...
	for (i32 c = startCol; c < endCol;)
	{
		...
		if (currentState != -1)
		{
			...
			else
			{
				...
				for (size_t i = c; i + endKw.size() <= endCol; i++)
				{
					bool found = true;
					...
					for (size_t i = c; i + kwStr.size() <= limit && i + kwStr.size() <= line.size(); i++)
					{
						...
						if (match)
						{
							...
							else
							{
								...
								if (bound && i + kwStr.size() < line.size())
								{
									u32 next = line[i + kwStr.size()];
		...
		if (measureEnd > (size_t)endCol)
			measureEnd = (size_t)endCol;
...
i32 MultilineTextInputState::getCharIndexAtPoint(const Point& pt)
{
	...
	if (visualLineIdx >= visualLines.size())
		visualLineIdx = (i32)visualLines.size() - 1;
...
void MultilineTextInputState::computeVisualLines(Font* font, f32 availableWidth, const RangeHighlight* rules, u32 ruleCount, const KeywordInfo* keywords, u32 keywordCount)
{
	...
	if (lines_processed > 0 || visualLines.empty())
	{
		...
		if (isWrapping)
			estimatedTotal = (size_t)(estimatedTotal * 1.2f);
	...
	for (size_t i = (size_t)start; i < lines.size(); ++i)
	{
		...
		for (size_t c = 0; c < line.size(); )
		{
			...
			if (currentState != -1)
			{
				...
				{
					for (size_t k = 0; k < endKw.size(); k++) if (line[c + k] != endKw[k])
					{
						match = false;
						break;
					}
					...
					if (match && !rules32[currentState].escape.empty())
					{
						...
						while (backIdx >= (i32)rules32[currentState].escape.size())
						{
							...
							{
								if (line[backIdx + k] != rules32[currentState].escape[k])
								{
									escMatch = false;
									break;
								}
			...
			else
			{
				...
				for (u32 r = 0; r < count; r++)
				{
					...
					if (c + startKw.size() > line.size())
						match = false;
					else for (size_t k = 0; k < startKw.size(); k++) if (line[c + k] != startKw[k])
					{
						match = false;
						break;
					}
	...
	const RangeHighlight* rules, u32 ruleCount, const KeywordInfo* keywords, u32 keywordCount)
{
	...
	for (size_t c = 0; c < line.size(); )
	{
		...
		if (currentState != -1)
		{
			...
			{
				currentState = -1;
				appendSegment((i32)line.size(), rules32[oldState].info->color);
				break;
			}
		...
		else
		{
			...
			for (u32 k = 0; k < keywords32.size(); k++)
			{
				...
				if (c + kw.size() <= line.size())
				{
					...
					else
					{
						...
						if (bound && c + kw.size() < line.size())
						{
							u32 next = line[c + (u32)kw.size()];
					...
					if (bound)
					{
						...
						for (size_t i = 0; i < kw.size(); i++) if (line[c + i] != kw[i])
						{
							match = false;
							break;
						}
			...
			if (!keywordMatched)
			{
				...
				for (u32 r = 0; r < rules32.size(); r++)
				{
					...
					if (c + startKw.size() > line.size())
						match = false;
					else for (size_t k = 0; k < startKw.size(); k++)
						if (line[c + k] != startKw[k])
						{
							match = false;
							break;
...
bool MultilineTextInputState::processEvent(const InputEvent& ev)
{
	...
	else if (ev.window == ctx->settings.services.getFocusedWindow())
	{
		if (mouseDown || selectingWithMouse)
		{
			...
			if (mouseDown)
			{
				if (currentLine != mouseDownSelectionStartLine
					|| caretColumn != mouseDownSelectionStartColumn)
					mouseMoved = true;
		...
		if (ev.mouse.button == MouseButton::Left)
		{
			...
			else
			{
				...
				if (idx >= n || std::isspace((int)line[idx]))
				{
					...
					else
					{
						...
						else
						{
							...
							while (end + 1 < n && !std::isspace((int)line[end + 1]))
								end++;
				...
				else
				{
					...
					while (start > 0 && !std::isspace((int)line[start - 1]))
						start--;
		...
		else
		{
			...
			else
			{
				...
				if (removed > 0)
				{
					...
					if ((size_t)removed > totalTextLength)
						totalTextLength = 0;
					else
						totalTextLength -= removed;
